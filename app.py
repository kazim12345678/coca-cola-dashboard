...
if st.button("💾 Save Production Data"):
    if plant not in data_store:
        data_store[plant] = {}
    data_store[plant][line] = {str(day): edited_data.iloc[day-1]["Production"] for day in days}
...
summary_data = pd.DataFrame({"Plant": plants, "Total Production": [sum(data_store.get(p, {}).get(l, {}).values()) for p in plants for l in lines]})
...
if st.sidebar.button("Download Report"):
    if report_type == "Excel":
        ...
        c = canvas.Canvas("production_report.pdf", pagesize=ps.letter)
